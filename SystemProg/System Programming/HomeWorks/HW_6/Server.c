 ...
 * Date       : May 12th 2013
 * File       : Bil244_101044009_midterm_SERVER_Thread.c
 *
 * Description: 
 *          Gets orders from clients that connected using thread. 
 ...
 *
 *  $ gcc -c pipeclient.c
 *  $ gcc pipeclient.o -o pipeclient -lpthread
 *  $ ./pipeclient fifoname
 *
 *********************************************************
 */
...
void *serveToClient(void *PID);
static void signalCatch(int sigNum);
int findTarget(char str1[][100], char *search);
int listTarget(char str1[][100], char *search);
...
int main (int argc, char *argv[]) 
{
  ...
  while(!doneFlag)
  {
    ...
    if(strcmp(buf,"IamClient")==0)
    {
      ...
      pthread_create(&thread[k],NULL,serveToClient,&clientPID);
      k++;
      /*
      pidNo = fork();
      if(pidNo == 0)
...
void *serveToClient(void *PID)
{
  ...
  sprintf(fifoName,"%sClientFifo",(char *)PID);
  ...
  while(1) //Waiting for orders from client
  {
    ...
    if(strcmp(bufz,"find")==0)
    {
      ...
      read(serveToClient,search,21);
      
      printf("Search : %s\n",search );

      //Search the target
      findTarget(listDirName,search);
    ...
    {
      sleep(1);
      printf("Thread is being killed..\n");
      read(serveToClient,search,21);

      printf("[%d] Thread Terminated \n\n",(int)pthread_self() );
      fprintf(logFilefork, "Thread Terminated..\n");
      pthread_exit(NULL);
...
int findTarget(char str1[][100], char *search)
{
  ...
  for (i = 0; i < strlen(str1[j]); ++i)
    if (strncmp (&str1[j][i], search, strlen(search)) == 0)
      count++;
